...
request.responseDbPromise = function (promise, db, event, context, callback, code = 200, headers = {}) {
  return promise
    .then(res =>
      db.end()
        .then(() => request.response(null, res, event, context, callback, code, headers))
    )
    .catch(err =>
      db.end()
...
request.responseDbAuthPromise = function (promise, db, event, context, callback, code = 200) {
  return promise
    .then(res =>
      db.end()
    ...
    .catch((err) => {
      if (err.code === 'ServiceUnavailableException') {
        return db.end()
          .then(() => request.response(error.unavailable(err), null, event, context, callback));
      }
      return db.end()
        .then(() => request.response(error.authError(err), null, event, context, callback));
    });
};
