...
Db.listAppsForVendor = (vendor, offset = 0, limit = 100) =>
  db.queryAsync(
    ...
    [vendor, limit ? _.toSafeInteger(limit) : 100, _.toSafeInteger(offset)]
  )
  .then((res) => {
    res.map((appIn) => {
      const app = appIn;
...
Db.listVersions = (id, offset = 0, limit = 100) =>
  db.queryAsync(
    'SELECT * ' +
    'FROM appVersions ' +
    'WHERE id = ? ' +
    'ORDER BY createdOn LIMIT ? OFFSET ?;',
    [id, limit ? _.toSafeInteger(limit) : 100, _.toSafeInteger(offset)]
  )
  .then((res) => {
...
Db.listVendors = (offset = 0, limit = 100) =>
  db.queryAsync(
    'SELECT id, name, address, email ' +
    'FROM vendors ' +
    'ORDER BY id LIMIT ? OFFSET ?;',
    [limit ? _.toSafeInteger(limit) : 100, _.toSafeInteger(offset)]
  );

Db.listApps = (filter, offset = 0, limit = 100) => {
  let filterSql = '';
...
Db.getPublishedApp = function (id, version) {
  let query;
  if (version) {
    query = db.queryAsync(
      ...
      'WHERE a.id=? AND a.version=? AND ap.isApproved=1;',
      [id, version]
    );
  } else {
    query = db.queryAsync(
      'SELECT a.id, a.name, a.vendor as vendorId, v.name as vendorName, ' +
      ...
      'a.forwardToken, a.uiOptions, a.testConfiguration, a.configurationSchema, ' +
      'a.configurationDescription, a.emptyConfiguration, a.actions, a.fees, ' +
      'a.limits, a.logger, a.loggerConfiguration, a.icon32, a.icon64, a.legacyUri ' +
      'FROM apps AS a ' +
      'LEFT JOIN vendors v ON (a.vendor = v.id) ' +
      'WHERE a.id=? AND a.isApproved=1;',
      id
    );
  }

  return query.spread((res) => {
    if (!res) {
      throw error.notFound();
    }
    return Db.formatAppOutput(res);
...
Db.listAllPublishedApps = (offset = 0, limit = 100) =>
  db.queryAsync(
    'SELECT id, vendor, name, version, type, shortDescription, icon32, ' +
    'icon64, legacyUri ' +
    'FROM apps ' +
...
Db.listPublishedAppVersions = (id, offset = 0, limit = 100) =>
  db.queryAsync(
    'SELECT a.id, a.name, ap.vendor as vendorId, v.name as vendorName, ' +
    'v.address as vendorAddress, v.email as vendorEmail, a.version, a.type, ' +
    'a.repoType, a.repoOptions, a.repoUri, a.repoTag, a.shortDescription, ' +
    ...
    'a.forwardToken, a.uiOptions, a.testConfiguration, a.configurationSchema, ' +
    'a.configurationDescription, a.emptyConfiguration, a.actions, a.fees, ' +
    'a.limits, a.logger, a.loggerConfiguration, a.icon32, a.icon64, ap.legacyUri ' +
    'FROM appVersions a ' +
    'LEFT JOIN apps ap ON (ap.id = a.id) ' +
    'LEFT JOIN vendors v ON (ap.vendor = v.id) ' +
