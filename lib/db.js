...
Db.formatAppOutput = function (app) {
  ...
  if (_.has(app, 'vendorId') && _.has(app, 'vendorName') && _.has(app, 'vendorAddress')
    && _.has(app, 'vendorEmail')) {
    app.vendor = {
      id: app.vendorId,
      name: app.vendorName,
...
Db.checkAppAccess = function (id, vendor, callback) {
  db.query('SELECT COUNT(*) as c FROM apps WHERE id = ? AND vendor = ?', [id, vendor], (err, result) => {
    if (err) {
      return callback(err);
    }
...
Db.updateApp = function (paramsIn, id, user, callbackMain) {
  ...
  async.waterfall([
    function (callback) {
      db.query('UPDATE apps SET ?, version = version + 1 WHERE id = ?', [params, id], err => callback(err));
    },
    function (callback) {
  ...
  async.waterfall([
    function (callback) {
      db.query(
        ...
        'WHERE id = ?', ['icon' + size, id + '/' + size + '/', '.png', id], err => callback(err));
    },
    function (callback) {
...
Db.getApp = function (id, version, callback) {
  async.waterfall([
    function (callbackLocal) {
      if (version) {
        db.query('SELECT * FROM appVersions WHERE id = ? AND version = ?', [id, version], (err, res) => callbackLocal(err, res));
      } else {
        db.query('SELECT * FROM apps WHERE id = ?', [id], (err, res) => callbackLocal(err, res));
      }
    },
