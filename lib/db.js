...
Db.connect = function(host, user, pass, dbname) {
...
Db.getApp = function(id, version, callback) {
  async.waterfall([
    ...
    function(data, callbackLocal) {
      ...
      return callbackLocal(null, formatAppOutput(data[0]));
    }
  ], callback);
};
...
Db.getPublishedApp = function(id, version, callback) {
  db.query(
    ...
    'a.configurationSchema, a.networking, a.actions, a.fees, a.limits, a.logger ' +
    'FROM apps AS a ' +
...
Db.listAllPublishedApps = function(offset, limit, callback) {
  ...
  db.query('SELECT id, vendor, name, version, type, shortDescription ' +
    'FROM apps ' +
    'WHERE isApproved = 1 ' +
