...
Db.listPublishedApps = (offset = 0, limit = defaultLimit, project = null) => {
  const allSql = 'SELECT id, vendor, name, version, type, ' +
  ...
  if (project) {
    sql = `(${allSql}) UNION ` +
    '(SELECT a.id, a.vendor, a.name, a.version, a.type, a.shortDescription, ' +
    'a.icon32, a.icon64, a.legacyUri ' +
    'FROM apps a ' +
    'LEFT JOIN appsProjects p ON (a.id = p.app) ' +
    'WHERE a.isApproved=1 AND a.isVisible=0 AND p.project=?) ';
    args.unshift(_.toSafeInteger(project));
  } else {
    sql = allSql;
  }
