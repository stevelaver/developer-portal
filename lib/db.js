...
Db.listApps = (offset = 0, limit = defaultLimit) =>
  db.queryAsync(
    `SELECT id, version, name, type, createdOn, createdBy, isPublic, legacyUri
    FROM apps
...
Db.listPublishedApps = (offset = 0, limit = defaultLimit) => {
  const sql = 'SELECT id, vendor, name, version, type, ' +
    ...
    'FROM apps ' +
    'WHERE isPublic=1 AND deletedOn IS NULL ' +
    'ORDER BY name LIMIT ? OFFSET ?;';
...
Db.listAppsForVendor = (vendor, offset = 0, limit = defaultLimit) =>
  db.queryAsync(
    'SELECT id, version, name, type, createdOn, createdBy, isPublic, ' +
    'legacyUri, icon32, icon64 ' +
    'FROM apps ' +
    'WHERE vendor = ? AND deletedOn IS NULL ' +
    'ORDER BY name LIMIT ? OFFSET ?;',
...
Db.getAppWithVendor = function (id, version, checkPublished = false, forAdmin = false) {
  ...
  if (version) {
    ...
    sql += 'a.limits, a.logger, a.loggerConfiguration, a.stagingStorageInput, a.icon32, ' +
      'a.icon64, a.legacyUri, a.isPublic, a.isDeprecated, a.expiredOn, a.replacementApp ' +
    ...
    sql = 'SELECT a.id, a.name, a.vendor as vendorId, v.name as vendorName, ' +
      ...
      'a.stagingStorageInput, a.icon32, a.icon64, a.legacyUri, ';
    if (forAdmin) {
      sql += 'a.permissions, ';
    }
    sql += 'a.isPublic, a.isDeprecated, a.expiredOn, a.replacementApp ' +
      'FROM apps AS a ' +
