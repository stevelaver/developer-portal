...
class DbUsers {
  ...
  deleteUser(username) {
    return this.rds.queryAsync('DELETE FROM users WHERE id=?', [username]);
  }
  ...
  addUserRequestToVendor(user, vendor) {
    return this.rds.queryAsync('INSERT INTO usersToVendorsRequests SET user=?, vendor=?', [user, vendor])
      .catch((err) => {
        if (_.startsWith(err.message, 'ER_DUP_ENTRY')) {
          throw this.err.badRequest('Already sent');
        }
        throw err;
      });
  }

  removeUserRequestToVendor(user, vendor) {
    return this.rds.queryAsync('DELETE FROM usersToVendorsRequests WHERE user=? AND vendor=?', [user, vendor]);
  }

  listUserRequestsForVendor(vendor, offset = 0, limit = 1000) {
    return this.rds.queryAsync(
      'SELECT user AS username, createdOn ' +
      'FROM usersToVendorsRequests ' +
      'WHERE vendor=? ' +
