...
class DbVendors {
  ...
  adminListAll(offset = 0, limit = 1000) {
    return this.rds.queryAsync(
      'SELECT id, name, address, email, isApproved, isPublic ' +
      'FROM vendors ' +
      'ORDER BY id LIMIT ? OFFSET ?;',
      [limit ? _.toSafeInteger(limit) : 1000, _.toSafeInteger(offset)]
    );
  ...
  publicGetVendor(id) {
    return this.rds.queryAsync('SELECT id, name, address, email FROM vendors WHERE id = ?', [id])
      .spread((res) => {
        if (!res) {
          throw this.err.notFound();
        }
