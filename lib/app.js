...
class App {
  ...
  insertApp(bodyIn, user) {
    ...
    body.vendor = user.vendors[0]; // TODO get vendor from parameter
    body.id = `${user.vendors[0]}.${body.id}`;
  ...
  insertAppByAdmin(bodyIn, user) {
    const body = JSON.parse(JSON.stringify(bodyIn));
    body.createdBy = user.email;
    body.id = `${body.vendor}.${body.id}`;

    return this.db.checkAppNotExists(body.id)
      .then(() => this.db.checkVendorExists(body.vendor))
      .then(() => this.db.insertApp(body))
      .then(() => null);
  }
  ...
  makeUserAdmin(cognito, email) {
    ...
    .then((data) => {
      ...
      if (Object.prototype.hasOwnProperty.call(attrs, 'custom:isAdmin') && attrs['custom:isAdmin'] === 1) {
  ...
  createVendor(cognito, body) {
    return cognito.createGroup({
      GroupName: body.id,
      UserPoolId: this.env.COGNITO_POOL_ID,
      Description: body.name,
    }).promise()
      .then(() => this.db.createVendor(body))
      .catch((err) => {
        if (_.startsWith('ER_DUP_ENTRY', err.message)) {
          throw this.err.badRequest('Vendor already exists');
        }
      })
      .then(() => null);
  }
