...
class App {
  ...
  getApp(id, vendor, version = null) {
    ...
    .then((appIn) => {
      appData = appIn;
      return this.db.getAppProjects(id);
    })
    .then(projects => _.map(projects, 'project'))
    .then((projects) => {
      appData.icon = {
        32: appData.icon32 ? `https://${this.env.CLOUDFRONT_URI}/${appData.icon32}` : null,
        64: appData.icon64 ? `https://${this.env.CLOUDFRONT_URI}/${appData.icon64}` : null,
      };
      delete appData.icon32;
      delete appData.icon64;
  ...
  static addIcons(input, cfUri) {
    const res = input;
    res.icon = {
      32: input.icon32 ? `https://${cfUri}/${input.icon32}` : null,
      64: input.icon64 ? `https://${cfUri}/${input.icon64}` : null,
    };
    delete res.icon32;
    delete res.icon64;
    return res;
  ...
  getPublishedApp(id, version = null) {
    return this.db.getPublishedApp(id, version)
    .then(data => App.addIcons(data, this.env.CLOUDFRONT_URI));
  }

  listPublishedApps(offset = 0, limit = 1000, project = null) {
    return this.db.listPublishedApps(offset, limit, project);
  }
