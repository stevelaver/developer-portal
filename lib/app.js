...
class App {
  ...
  listAppChanges(diff, since = null, until = null) {
    // TODO pagination
          ...
          promises.push(new Promise(resolve =>
            this.db.getAppVersion(app.id, app.version - 1)
              ...
              .then((prev) => {
                ...
                const nextCopy = _.cloneDeep(this.db.formatAppOutput(app));
                ...
                const difference = diff(prevCopy, nextCopy);
                resolve({
                  id: app.id,
                  createdOn: app.createdOn,
                  createdBy: app.createdBy,
                  changes: difference ? difference.map((item) => {
                    let operation;
                    switch (item.kind) {
                      case 'N':
                        operation = 'added';
                    ...
                    return {
                      attribute: item.path.join('.'),
                      oldValue: _.get(item, 'lhs', null),
                      newValue: _.get(item, 'rhs', null),
                      description: `Attribute ${item.path.join('.')} ${operation}`,
                    };
                  }) : [],
                });
              })
