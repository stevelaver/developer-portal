...
class App {
  ...
  listApps(filter, offset = 0, limit = 100) {
  ...
  getPublishedApp(id, version = null) {
    ...
    .then(data => app.addIcons(data, app.env.CLOUDFRONT_URI));
  }

  listPublishedApps(offset = 0, limit = 100) {
    return this.db.listAllPublishedApps(offset, limit);
  }

  listPublishedAppVersions(id, offset = 0, limit = 100) {
    const app = this;
    return this.db.listPublishedAppVersions(id, offset, limit)
    .then(res => res.map(r => app.addIcons(r, app.env.CLOUDFRONT_URI)));
  }

  listVendors(offset = 0, limit = 100) {
