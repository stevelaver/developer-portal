...
class Validation {
    ...
    if (_.has(def, 'path')) {
      if (_.isArray(def.path)) {
        _.forEach(def.path, (item) => {
          switch (item) {
            case 'vendor':
              path.vendor = this.joi.string().required().error(Error('Path parameter vendor is required and must be string'));
              break;
            default:
              path[item] = this.joi.string().required().error(Error(`Path parameter ${item} is required and must be string`));
          }
        });
      } else {
  ...
  commonAppSchema() {
    return {
      ...
      repository: this.joi.object().keys({
        type: this.joi.string().valid('dockerhub', 'quay', 'builder', 'ecr')
          .error(Error('Parameter repository.type must be one of: dockerhub, quay, builder, ecr')),
      ...
      network: this.joi.string().valid('none', 'bridge')
        .error(Error('Parameter network must be one of: none, bridge')),
  ...
  adminCreateVendor() {
    return {
      id: this.joi.string().max(32)
        .error(Error('Parameter id is required and may have 32 characters at most')),
      name: this.joi.string().max(128)
        .error(Error('Parameter name is required and may have 128 characters at most')),
      address: this.joi.string().max(255)
        .error(Error('Parameter address is required and may have 255 characters at most')),
      email: this.joi.string().email().max(128)
