...
class Validation {
    ...
    if (_.has(def, 'query')) {
      if (_.isArray(def.query)) {
        _.forEach(def.query, (item) => {
          switch (item) {
            case 'email':
              query.email = this.joi.string().email().required()
                .error(Error('Parameter email in query must be valid email address'));
              break;
            case 'version':
              query.version = this.joi.number().integer().required()
                .error(Error('Parameter version in query must be an integer'));
              break;
            default:
              query[item] = this.validateString(`${item} in query`).required();
          }
        });
      } else {
  ...
  commonAppSchema() {
    return {
      ...
      repository: this.validateObject('repository').keys({
